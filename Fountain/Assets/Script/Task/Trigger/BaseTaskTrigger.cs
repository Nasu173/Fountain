...
public abstract class BaseTaskTrigger : MonoBehaviour, ITaskTrigger
{
    ...
    protected virtual void Update()
    {
        ...
        if (!taskCompleted && CheckTriggerCondition())
        {
            ...
            {
                UpdateTaskProgress(); // 已启动，更新进度
            }
        }
    }

    /// <summary>
    /// 启动任务，向TaskManager注册任务
    /// </summary>
    ...
    protected virtual void OnTaskCompleted()
    {
        ...
        if (nextTaskTrigger != null)
        {
            if (nextTaskDelay > 0)
            {
                Invoke(nameof(StartNextTask), nextTaskDelay); // 延迟触发
            }
            else
            {
                StartNextTask(); // 立即触发
    ...
    private void StartNextTask()
    {
        if (nextTaskTrigger != null)
        {
            // 通过反射调用下一个任务的StartTask（因为protected不能直接调用）
